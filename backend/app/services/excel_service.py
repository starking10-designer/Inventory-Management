...
def get_flipkart_target_date(dispatch_period: str = None) -> date:
    now = datetime.now()
...
def filter_flipkart_orders(file_path, dispatch_period: str = None):
    ...
    target_date = get_flipkart_target_date(dispatch_period).strftime("%Y-%m-%d")
...
def filter_flipkart_warehouse_orders(file_path):
    ...
    if missing_columns:
        raise ValueError(
            "Flipkart Warehouse file must include "
    ...
    for _, row in df.iterrows():
        order_status = str(row.get("orderstatus", "")).strip().upper()
        ...
        order_id = str(row.get("itemid", "")).strip()
        sku = str(row.get("sku", "")).strip()

        if not sku or pd.isna(order_date):
            continue
        ...
        if order_id:
            ...
            fallback_index += 1
            order_id = f"flipkart-warehouse-row-{fallback_index}"
...
def generate_daily_report(expanded_inventory, db):
    ...
    for item in expanded_inventory:
        ...
        return_inventory = db.query(
            ...
            func.lower(ReturnInventory.style) == str(style or "").strip().lower(), func.lower(ReturnInventory.color) == str(color or "").strip().lower(), func.lower(ReturnInventory.size) == str(size or "").strip().lower()
        ).first()
