...
const PLATFORM_BADGES = {
  ...
  "Flipkart Warehouse": "bg-sky-100 text-sky-800",
};
...
function mergePlatformSummary(target, source) {
  ...
  if (existing) {
    ...
    existing.total_invoice_amount += Number(source.total_invoice_amount || 0);
    return;
  }
...
function groupReportsByDate(rawReports) {
  ...
  return [...byDate.values()]
    .map((report) => {
      report.platforms.sort((a, b) => a.platform.localeCompare(b.platform));
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            >
              {isDownload ? "Download sales report" : "Filter sales reports"}
            </h2>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          >
            ...
            {busy ? "Generating..." : isDownload ? "Download" : "Filter"}
          </button>
        </div>
      </div>
    </div>
...
export default function SalesReportsPage() {
  ...
  const visibleReports = useMemo(
    () =>
      groupedReports.filter((report) => {
        if (filterFromDate && report.report_date < filterFromDate) return false;
        if (filterToDate && report.report_date > filterToDate) return false;
        return true;
      }),
    [groupedReports, filterFromDate, filterToDate],
  );

  const openDateRange = (action) => {
    const dates = groupedReports.map((report) => report.report_date).sort();
    setRangeFromDate(filterFromDate || dates[0] || "");
    setRangeToDate(filterToDate || dates[dates.length - 1] || "");
  ...
  const downloadRangeReport = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE}/sales-analytics/export-range`,
        {
          params: {
      ...
      link.setAttribute(
        "download",
        `sales_report_${rangeFromDate}_to_${rangeToDate}.xlsx`,
      );
  ...
  const confirmDateRange = async () => {
    ...
    if (rangeAction === "filter") {
      setFilterFromDate(rangeFromDate);
      setFilterToDate(rangeToDate);
      setRangeAction(null);
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-white to-sky-100 text-slate-800">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <button
            type="button"
            onClick={() => openDateRange("filter")}
            disabled={loading || groupedReports.length === 0}
            className="inline-flex items-center gap-2 rounded-xl border border-violet-200 bg-white px-4 py-2.5 text-sm font-semibold text-violet-700 shadow-sm hover:bg-violet-50 disabled:opacity-50"
          >
            <ListFilter size={17} />
            Filter
          </button>
          <button
            type="button"
            onClick={() => openDateRange("download")}
            disabled={loading || groupedReports.length === 0}
          ...
          {(filterFromDate || filterToDate) && (
            <div className="inline-flex flex-wrap items-center gap-2 rounded-xl bg-violet-50 px-3 py-2 text-sm text-violet-800">
              <span>
              ...
              <button
                ...
                className="font-semibold hover:text-violet-950"
              >
                Clear
              </button>
            </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-white/80 bg-white/55 backdrop-blur-xl shadow-sm px-6 py-16 text-center text-slate-500">
            {groupedReports.length === 0
              ? "No sales reports saved yet. Generate a final report first to record platform sales."
              : "No sales reports found in the selected date range."}
          </div>
        ) : (
              ...
              >
                ...
                <div className="border-t border-slate-200/80 px-5 py-3 bg-white/60">
                  <button
                    ...
                    disabled={downloadingKey === `${report.report_date}-All`}
        ...
        {!loading && visibleReports.length > 0 && (
          <p className="mt-4 text-xs text-slate-500">
            {visibleReports.length} dated sales report
            {visibleReports.length === 1 ? "" : "s"} available
          </p>
        )}
      </div>
      ...
      <DateRangeModal
        ...
        toDate={rangeToDate}
        onFromDateChange={setRangeFromDate}
        onToDateChange={setRangeToDate}
        onClose={() => setRangeAction(null)}
        onConfirm={confirmDateRange}
        busy={rangeDownloading}
      />
    </div>
  );
}
